...
abstract class PropertiesFile(val storagePath: File, val comments: String) {
  ...
  protected val props = new Properties() {
    ...
    //Override to make output sorted by keys
    override def keys(): Enumeration[java.lang.Object] = {
      val keysEnum = super.keys();
      var list = Nil: List[String]
      while (keysEnum.hasMoreElements()) {
        list = keysEnum.nextElement().toString :: list
      }
      val sortedList = list.sortWith(_ < _)
      val enumToReturn: Enumeration[java.lang.Object] =
        asJavaEnumeration(sortedList.map(_.asInstanceOf[java.lang.Object]).iterator)
      enumToReturn
    }
